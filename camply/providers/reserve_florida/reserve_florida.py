...
from camply.config.api_config import ReserveFloridaConfig
...
from camply.containers.reserve_florida import (
    ReserveFloridaAvailabilityResponse,
    ReserveFloridaAvailabilitySlice,
    ReserveFloridaAvailabilityUnit,
    ReserveFloridaCityPark,
    ReserveFloridaDetailedPlace,
    ReserveFloridaFacilityMetadata,
    ReserveFloridaMetadata,
)
...
class ReserveFlorida(BaseProvider):
    """
    Camply Provider for Reserve Florida
    """

    reserve_florida_city_parks: Dict[int, ReserveFloridaCityPark] = {}
    reserve_florida_rec_areas: Dict[int, RecreationArea] = {}
    reserve_florida_campgrounds: Dict[int, CampgroundFacility] = {}
    reserve_florida_unit_categories: Dict[int, str] = {}
    reserve_florida_unit_type_groups: Dict[int, str] = {}
    metadata_refreshed: bool = False
    active_search: bool = False
    offline_cache_dir = FileConfig.RESERVE_FLORIDA_PROVIDER
    ...
    def refresh_metadata(self) -> None:
        ...
        as JSON alongside the provider code itself (floridardr.usedirect.com):
    ...
    def search_for_recreation_areas(
        ...
        """
        if state.upper() != "FL":
            raise CamplyError("ReserveFlorida doesn't support states outside FL")
        if query is None:
            logger.error(
                "You must provide a search string to search `ReserveFlorida` Recreation Areas"
            )
            sys.exit(1)
        ...
        found_recareas = [
            rec_area
            for rec_area in self.reserve_florida_rec_areas.values()
    ...
    def _search_for_campgrounds(
        ...
        if len(campground_id) >= 1:
            for camp_id in campground_id:
                found_campgrounds += [
                    campground
                    for campground in self.reserve_florida_campgrounds.values()
    ...
    def get_campsites(
        ...
        """
        Get Campsites from ReserveFlorida
        ...
        data = {
            ...
            "StartDate": start_date.strftime(ReserveFloridaConfig.DATE_FORMAT),
            "WebOnly": web_only,
            "UnitTypesGroupIds": []
            if unit_type_group_ids is None
            else unit_type_group_ids,
            "SleepingUnitId": sleeping_unit_id,
            "EndDate": end_date.strftime(ReserveFloridaConfig.DATE_FORMAT),
            "UnitSort": unit_sort,
        ...
        url = f"{ReserveFloridaConfig.BASE_URL}/{ReserveFloridaConfig.AVAILABILITY_ENDPOINT}"
        response = self.session.post(
            url=url, data=json.dumps(non_null_data), headers=self.json_headers
        )
        response.raise_for_status()
        availability_response = ReserveFloridaAvailabilityResponse(**response.json())
    ...
    def _get_available_campsite(
        self,
        availability_slice: ReserveFloridaAvailabilitySlice,
        availability_response: ReserveFloridaAvailabilityResponse,
        unit: ReserveFloridaAvailabilityUnit,
    ) -> AvailableCampsite:
        """
        ...
        facility = self.reserve_florida_campgrounds[facility_id]
        recreation_area = self.reserve_florida_rec_areas[facility.recreation_area_id]
        booking_url = (
            f"{ReserveFloridaConfig.CAMPGROUND_URL}/Web/Default.aspx#!park/"
        ...
        campsite_type = self.reserve_florida_unit_categories.get(
            unit.UnitCategoryId, None
        )
        campsite_use_type = self.reserve_florida_unit_type_groups.get(
    ...
    def _fetch_metadata_from_disk(
        ...
        if file_path.exists() is False:
            ...
            if (
                ...
                data = json_body
        if data is None:
            logger.debug("Refreshing ReserveFlorida Metadata: %s", file_path.name)
        return data

    def _get_campground_metadata(self) -> ReserveFloridaMetadata:
        """
    ...
    def _get_city_parks(self) -> Dict[int, ReserveFloridaCityPark]:
        ...
        if city_park_data is None:
            url = f"{ReserveFloridaConfig.BASE_URL}/{ReserveFloridaConfig.CITYPARK_ENDPOINT}"
            resp = self.session.get(url=url)
            resp.raise_for_status()
            city_park_data: Dict[str, Dict[str, Any]] = resp.json()
            metadata_file.write_text(json.dumps(city_park_data, indent=2))
        self.reserve_florida_city_parks: Dict[int, ReserveFloridaCityPark] = {
            int(city_park_id): ReserveFloridaCityPark(**city_park_json)
            for city_park_id, city_park_json in city_park_data.items()
        }
        return self.reserve_florida_city_parks

    def _get_places(self) -> Dict[int, ReserveFloridaDetailedPlace]:
        """
        Fetch Metadata On Every Place

        Returns
        -------
        Dict[int, ReserveFloridaDetailedPlace]
        """
        ...
        if places_data is None:
            url = (
                f"{ReserveFloridaConfig.BASE_URL}/"
                f"{ReserveFloridaConfig.LIST_PLACES_ENDPOINT}"
            )
            resp = self.session.get(url=url)
            resp.raise_for_status()
        ...
        places_validated = [
            ReserveFloridaDetailedPlace(**place_json) for place_json in places_data
        ]
        places_data_validated: Dict[int, ReserveFloridaDetailedPlace] = {
            item.PlaceId: item for item in places_validated
        }
        self.reserve_florida_rec_areas: Dict[int, RecreationArea] = {
    ...
    def _get_facilities(self) -> Dict[int, ReserveFloridaFacilityMetadata]:
        """
        Fetch Metadata On Every Facility

        Returns
        -------
        Dict[int, ReserveFloridaFacilityMetadata]
        """
        ...
        if facilities_data is None:
            url = (
                f"{ReserveFloridaConfig.BASE_URL}/"
                f"{ReserveFloridaConfig.LIST_FACILITIES_ENDPOINT}"
            )
            resp = self.session.get(url=url)
            resp.raise_for_status()
        ...
        facilities_validated = [
            ReserveFloridaFacilityMetadata(**facility_json)
            for facility_json in facilities_data
        ]
        facilities_data_validated: Dict[int, ReserveFloridaFacilityMetadata] = {
            item.FacilityId: item for item in facilities_validated
        }
        self.reserve_florida_campgrounds: Dict[int, CampgroundFacility] = {}
        for facility in facilities_data_validated.values():
            rec_area = self.reserve_florida_rec_areas.get(facility.PlaceId, None)
            if rec_area is not None:
                self.reserve_florida_campgrounds[
                    facility.FacilityId
                ] = CampgroundFacility(
