"""
Search Implementation: Reserve Florida
"""
...
from camply.providers.reserve_florida import ReserveFlorida
...
class SearchReserveFlorida(BaseCampingSearch):
    """
    Searches on ReserveFlorida.com for Campsites
    """

    provider_class = ReserveFlorida
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.campsite_finder: ReserveFlorida
        ...
        try:
            ...
            logger.error(
                "You must provide a Campground ID or a Recreation Area ID to ReserveFlorida"
            )
            sys.exit(1)
        ...
        if kwargs.get("equipment", ()):
            logger.warning("ReserveFlorida Doesn't Support Equipment, yet 🙂")

    def get_all_campsites(self, **kwargs: Dict[str, Any]) -> List[AvailableCampsite]:
        """
        Retrieve All Campsites from the ReserveFlorida API
    ...
    def find_recreation_areas(
        ...
        """
        Return the ReserveFlorida Recreation Areas
        """
        rec_areas = cls.provider_class().search_for_recreation_areas(
            query=search_string, state=kwargs.get("state", "FL")
        )
